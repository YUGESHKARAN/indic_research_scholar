  ...
  return (
    <div className="min-h-screen bg-[#10101B] text-[#F1EEE4] font-body">
      ...
      <section className="max-w-6xl mx-auto px-6 pt-20 pb-24">
        <p className="font-mono text-xs tracking-[0.2em] text-[#E8A33D] uppercase mb-6">
        ...
        <p className="font-body text-lg text-[#F1EEE4]/70 max-w-xl mt-6 leading-relaxed">
        ...
        <div className="mt-12 flex flex-wrap items-baseline gap-x-5 gap-y-3">
          {SCRIPT_PARADE.map((s, i) => (
            <React.Fragment key={s.word}>
              <span
                className="text-2xl md:text-3xl text-[#F1EEE4]/90"
        ...
        <div className="flex flex-wrap gap-3 mt-10">
          ...
          <Link
            to="/ask"
            className="px-6 py-3 rounded-md border border-[#2A2A3D] text-[#F1EEE4]
      ...
      {/* How it works */}
      <section className="border-t border-[#2A2A3D] bg-[#0D0D16]">
        <div className="max-w-6xl mx-auto px-6 py-20">
          ...
          <div className="grid md:grid-cols-2 gap-8 relative">
            ...
            {STEPS.map((step) => (
              <div
                key={step.number}
                className="bg-[#181826] border border-[#2A2A3D] rounded-xl p-8 flex flex-col"
              >
                <span className="font-mono text-sm text-[#E8A33D] mb-4">{step.number}</span>
                <h3 className="font-display text-2xl font-semibold mb-3">{step.title}</h3>
                <p className="text-[#F1EEE4]/60 leading-relaxed mb-6 flex-1">{step.body}</p>
                <Link
                  to={step.to}
                  className="font-body text-sm font-medium text-[#E8A33D] hover:text-[#f0b158]
